...
% The function is from Matlab Central Fileexchange (by Daniel Dougherty)
% https://it.mathworks.com/matlabcentral/fileexchange/
%-------------------------------------------------------------------------%
%
% File: haltonseq(NUMPTS,NDIMS)
%
% Goal: computes Halton nodes
%
% Inputs: NUMPTS:  number of Halton data
%         NDIMS: the space dimension
%
% Output: H:  the Halton data
%
%-------------------------------------------------------------------------%

function H = haltonseq(NUMPTS,NDIMS)
% HALTONSEQ(NUMPTS,NDIMS,) Generate a Halton sequence in NDIMS dimensional
%   space containing NUMPTS.  The output is between 0 and 1.  NUMPTS may
%   be a vector of integers in which they indicate which elements of
%   the Halton sequence to compute.  A vector NUMPTS is useful in
%   sequential Halton sequence sampling.
...
%dec2base can handle
% if (NDIMS > length(P))
% 	error(['Sorry, Halton sequences are only possible for up to ...
%   ',num2str(length(P)),' dimensions.']);
% else
...
for i = 1:NDIMS %Generate the components for each dimension.
    ...
    %V = V-'0'; %Converts string to a matrix of doubles with correct
    %numeric values.
